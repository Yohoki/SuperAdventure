...
namespace SuperAdventure
{
    public partial class TradingScreen : Form
    {
        ...
        public TradingScreen(Player player)
        {
            ...
            dgvMyInventory.RowHeadersVisible = false;
            dgvMyInventory.AutoGenerateColumns = false;

            // This hidden column holds the item ID, so we know which item to sell
            dgvMyInventory.Columns.Add(new DataGridViewTextBoxColumn
            {
            ...
            dgvMyInventory.Columns.Add(new DataGridViewButtonColumn
            {
                Text = "Sell 1",
            ...
            dgvMyInventory.DataSource = _currentPlayer.Inventory;

            // When the user clicks on a row, call this function
            dgvMyInventory.CellClick += dgvMyInventory_CellClick;


            // Populate the datagrid for the vendor's inventory
            dgvVendorInventory.RowHeadersVisible = false;
            dgvVendorInventory.AutoGenerateColumns = false;
            ...
            dgvVendorInventory.DataSource = _currentPlayer.CurrentLocation.VendorWorkingHere.Inventory;

            // When the user clicks on a row, call this function
            dgvVendorInventory.CellClick += dgvVendorInventory_CellClick;
        }

        private void dgvMyInventory_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                var itemID = dgvMyInventory.Rows[e.RowIndex].Cells[0].Value;
